...
<head>
    ...
    <style>
        ...
        .quick-action .btn { border-radius: 10px; }
    </style>
</head>
<body>
<div class="d-flex admin-wrapper">
    <aside class="sidebar d-flex flex-column">
        ...
        <nav class="nav flex-column py-3">
            <a class="nav-link active" href="${pageContext.request.contextPath}/admin/dashboard"><i class="bi bi-house-door me-2"></i>Dashboard</a>
...
<a class="nav-link" href="${pageContext.request.contextPath}/admin/reports"><i class="bi bi-cash-coin me-2"></i>Doanh thu</a>
    ...
    <main class="content-area">
        <div class="topbar d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0">Car Rental - Admin Dashboard</h5>
                <small class="text-muted">Tổng quan vận hành hệ thống</small>
            </div>
            <div class="text-end">
                <div class="fw-semibold">Xin chào, Admin</div>
                <small class="text-muted">${pageContext.request.serverName}</small>
            </div>
        </div>
        ...
        <div class="dashboard-body">
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-3">
                    <div class="card stat-card">
                        <div class="card-body">
                            <div class="text-muted small">Xe đang hoạt động</div>
                            <h4 class="mb-0">${activeCars}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card stat-card">
                        <div class="card-body">
                            ...
                            <h4 class="mb-0">${todayBookings}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
            ...
            <div class="row g-3">
                ...
                <div class="col-lg-4">
                    <div class="card chart-card">
                        <div class="card-header bg-white fw-semibold">Tỷ lệ trạng thái xe</div>
                        <div class="card-body">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <c:choose>
                                                        ...
                                                        <c:when test="${r.paymentStatus == 'Success'}">
                        ...
                        <ul class="list-group list-group-flush">
                            <c:forEach var="r" items="${latestReviews}">
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>"${r.comment}" - ${r.reviewerName}</span>
                                    <span>⭐ ${r.rating}</span>
                                </li>
                            </c:forEach>
                            <c:if test="${empty latestReviews}">
                                <li class="list-group-item text-muted">Chưa có đánh giá gần đây.</li>
                            </c:if>
                        </ul>
                    </div>
                </div>
...
<script>
    const revenueLabels = [
        <c:forEach var="p" items="${revenuePoints}" varStatus="loop">
            "${p.label}"<c:if test="${!loop.last}">,</c:if>
        </c:forEach>
    ];
    const revenueData = [
        <c:forEach var="p" items="${revenuePoints}" varStatus="loop">
            ${p.totalAmount}<c:if test="${!loop.last}">,</c:if>
        </c:forEach>
    ];
    ...
    const statusLabels = [
        <c:forEach var="s" items="${carStatusCounts}" varStatus="loop">
            "${s.statusName}"<c:if test="${!loop.last}">,</c:if>
        </c:forEach>
    ];
    const statusData = [
        <c:forEach var="s" items="${carStatusCounts}" varStatus="loop">
            ${s.totalCount}<c:if test="${!loop.last}">,</c:if>
        </c:forEach>
    ];

    const statusCtx = document.getElementById('carStatusChart');
    new Chart(statusCtx, {
        type: 'doughnut',
        data: {
            labels: statusLabels,
